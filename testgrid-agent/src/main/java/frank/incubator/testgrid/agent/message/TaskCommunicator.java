...
public class TaskCommunicator extends MessageListenerAdapter {
	...
	public void onMessage( final Message message ) {
		try {
			...
			switch ( type ) {
				case Constants.MSG_TASK_RESERVE:
					final Task task = CommonUtils.fromJson( getProperty( message, Constants.MSG_HEAD_TASK, "" ),
							Task.class );
					task.deleteObservers();
					final Test test = CommonUtils.fromJson( getProperty( message, Constants.MSG_HEAD_TEST, "" ),
							Test.class );
					test.deleteObservers();
					test.addObserver( agent.getNotifier() );
					log.info("Got request from:" + from + " to reserve devices for test:" + test.getId() + ", requirement:" + task.getRequirements());
					if ( agent.reserveForTest( test, task.getRequirements() ) ) {
						...
						setProperty( msg, Constants.MSG_HEAD_FROM,
								Constants.MSG_TARGET_AGENT + CommonUtils.getHostName() );
				...
				case Constants.MSG_TASK_CANCEL:
					...
					agent.cancelTest( testID, reason );
					break;
				case Constants.MSG_TEST_FINISH_CONFIRM:
