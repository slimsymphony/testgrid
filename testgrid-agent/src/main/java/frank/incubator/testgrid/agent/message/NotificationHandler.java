					...
					switch ( action ) {
						...
						case Constants.ACTION_DEVICE_RESERVE:
							...
							for ( Device device : devices ) {
								...
								device.setState( Device.DEVICE_BUSY );
				...
				case Constants.NOTIFICATION_TEST:
					...
					switch ( action ) {
						...
						case Constants.ACTION_TEST_CANCEL:
							agent.cancelTest( testId, "Cancelled by Notification from:" + from );
							break;
						default:
						...
						case Constants.ACTION_AGENT_PLUGIN_CONFIG:
							...
							if( plugin != null ) {
								...
								if ( msg.propertyExists( Constants.MSG_HEAD_PLUGIN_ATTRIBUTES ) ) {
									Map<String,Object> atts = CommonUtils.fromJson( getProperty( msg, Constants.MSG_HEAD_PLUGIN_ATTRIBUTES, "" ), new TypeToken<Map<String,Object>>(){}.getType() );
