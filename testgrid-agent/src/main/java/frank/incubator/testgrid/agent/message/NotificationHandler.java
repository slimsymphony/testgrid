...
public class NotificationHandler extends MessageListenerAdapter {
	...
	public void handleMessage(Message msg) {
		...
		try {
			...
			log.info("Current Agent received a Notification Message coming from :" + from + ", Type:" + mode + ", action:" + action);
			switch (mode) {
				case Constants.NOTIFICATION_DEVICE:
					String deviceCondition = getProperty(msg, Constants.MSG_HEAD_DEVICE_CONDITION, "");
					Map<String, Object> conditions = CommonUtils.fromJson(deviceCondition, new TypeToken<Map<String, Object>>() {
					}.getType());
					...
					switch (action) {
						...
						case Constants.ACTION_DEVICE_RESERVE:
							...
							for (Device device : devices) {
								device = agent.getDm().getDeviceById(device.getId());
								device.setRole(Device.ROLE_MAIN);
								agent.getDm().updateDeviceState(device, Device.DEVICE_RESERVED, testId, true);
						...
						case Constants.ACTION_DEVICE_OFFLINE:
							for (Device device : devices) {
								device = agent.getDm().getDeviceById(device.getId());
								device.setRole(Device.ROLE_MAIN);
								agent.getDm().updateDeviceState(device, Device.DEVICE_LOST, "", true);
				...
				case Constants.NOTIFICATION_SYSTEM:
					switch (action) {
						...
						case Constants.ACTION_AGENT_CONFIG_UPDATE:
							...
							Properties props = CommonUtils.fromJson(getProperty(msg, Constants.MSG_HEAD_CONFIG_CHANGES, ""), Properties.class);
						...
						case Constants.ACTION_AGENT_PLUGIN_CONFIG:
							...
							if (plugin != null) {
								...
								if (msg.propertyExists(Constants.MSG_HEAD_PLUGIN_ATTRIBUTES)) {
									Map<String, Object> atts = CommonUtils.fromJson(getProperty(msg, Constants.MSG_HEAD_PLUGIN_ATTRIBUTES, ""),
