		...
		for( Device d : this.getDm().listDevices() ) {
			try {
				if( d instanceof AndroidDevice && d.getState() != Device.DEVICE_LOST && d.getState() != Device.DEVICE_LOST_TEMP ) {
					String sn = d.getAttribte( Constants.DEVICE_SN );
					...
					try {
						...
						String val = d.getAttribte( Constants.DEVICE_SIM1_SIGNAL );
						String val2 = d.getAttribte( Constants.DEVICE_SIM1_SIGNAL );
						if( val != null ) {
