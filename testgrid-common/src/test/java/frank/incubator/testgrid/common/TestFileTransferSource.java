...
public class TestFileTransferSource {
	...
	public static void testSocket(String[] args) throws Exception {
		...
		source.push(token, new ArrayList<File>() {
			{
				this.add(new File("a.txt"));
			}
		});
		source.dispose();
	}
	...
	public static void testHttp() throws Exception {
		...
		FileTransferTarget target = new HttpFileTransferTarget(FileTransferMode.TARGET_HOST, null, new File(
				"C:/develop/eclipse-workspaces/java8_workspace/testgrid/testgrid-agent/workspace"), System.out);
