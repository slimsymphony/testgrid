...
public class FtServiceSender extends MessageListenerAdapter {
	...
	public static void main( String[] args ) throws Exception {
		...
		FileTransferTask ftTask = new FileTransferTask( Constants.MSG_TARGET_CLIENT+CommonUtils.getHostName(), Constants.MSG_TARGET_AGENT+CommonUtils.getHostName(), "task1", "test1", files );
		InputStream in = CommonUtils.loadResources( "../testgrid-agent/" + Constants.MQ_CONFIG_FILE, true );
		StringWriter sw = new StringWriter();
		IOUtils.copy( in, sw );
		BrokerDescriptor[] bds = CommonUtils.fromJson( sw.toString(), new TypeToken<BrokerDescriptor[]>(){}.getType() );
		...
		hub.bindHandlers( Constants.BROKER_FT, Queue.class, Constants.HUB_FILE_TRANSFER, Constants.MSG_HEAD_TARGET+"='" + Constants.MSG_TARGET_CLIENT+CommonUtils.getHostName()+"' AND " + Constants.MSG_HEAD_TASKID+"="+"'" + ftTask.getTaskId() +"'", fts, System.out );
		hub.bindHandlers( Constants.BROKER_TASK, Queue.class, Constants.HUB_TASK_COMMUNICATION, Constants.MSG_HEAD_TARGET+"='" + Constants.MSG_TARGET_CLIENT+CommonUtils.getHostName()+"'", new SimpleMessageListener(fts), System.out );
		fts.sendTo( ftTask );
	}
