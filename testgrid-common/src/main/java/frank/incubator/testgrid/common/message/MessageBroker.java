...
/**
 * Represent a MessageQueue Broker. It's a channel with represent a unique port/way to communicate with MessageQueue service.
 * 
 * @author Wang Frank
 *
 */
public class MessageBroker extends BaseObject implements ExceptionListener {
	...
	public void establishConnection() throws MessageException {
		...
		while ( true ) {
			try {
				...
				if ( retryTimes == 10 ) { 
					throw new MessageException( "Create MessageBroker failed. Can't connect to Message Service[" + uri + "].", e );
				}
			}
		}
	}
	 ...
	 */
	public Pipe bindHandlers( Type type, String name, String messageSelector, MessageListener listener, MessageFilter ... filters )
			throws MessageException {
