...
/**
 * Common Log Utilities.
 * This class provides 2 types of log. LogConnector and Slf4j Loggers.
 * It will Hold on all the created log instances so it won't create multiple times.
 * 
 * @author Wang Frank
 *
 */
public class LogUtils {
	...
	public static LogConnector get( String name, Pipe pipe, Map<String,Object> criteria ) {
		...
		log = new LogConnector( logger, new BufferedMessageOutputStream(
					pipe.getParentBroker().getSession(), pipe.getProducer(), criteria ) );
		return log;
	}
	...
	private static Logger dynamic( String name, LogLevel level ) {
		...
		if( thName != null && !thName.trim().isEmpty() )
			name = name + "_" + thName.trim().replaceAll( " ", "_" );*/
		...
		if( !workspace.isEmpty() && !workspace.endsWith( "/" ) )
			workspace += "/";
		timeBasedRollingPolicy.setFileNamePattern( workspace + "logs/" + name + ".log.%d{yyyy-MM-dd}" );
		...
		rollingFileAppender.setPrudent( true ); // set to true to release the hold of log files.
