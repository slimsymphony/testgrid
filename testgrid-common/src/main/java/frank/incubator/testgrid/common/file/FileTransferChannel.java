...
/**
 * File Transfer channel defined a contract which represent the way to transfer
 * bunch of files. And it also knows how to select and invoke the concrete
 * FileTransferClass instance to do the actual work.
 * 
 * @author Wang Frank
 * 
 */
public abstract class FileTransferChannel extends BaseObject implements Comparable<FileTransferChannel> {
	...
	/**
	 * Priority defined the order. Higher priority will be sorted as smaller
	 * index.
	 * 
	 ...
	 * @return
	 */
	abstract public boolean validate(LogConnector log);
	 ...
	 * @return
	 */
	abstract public boolean apply(LogConnector log);
