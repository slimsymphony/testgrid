...
public class MessageHub {
	...
	public MessageHub(String hostType, BrokerDescriptor... descs)
			throws MessageException {
		for (BrokerDescriptor desc : descs) {
			if (desc != null) {
				brokers.put(
						desc.getId(),
						new MessageBroker(desc.getId(), desc.getUri(), desc
								.getMq(), hostType));
			}
		}
	...
	public MessageHub(String uri, String hostType) throws MessageException {
		this.hostType = hostType;
		brokers.put(DEFAULT_BROKER, new MessageBroker(DEFAULT_BROKER, uri,
				MqBuilder.ActiveMQ, hostType));
	}
	...
	public void addBroker(String brokerName, String uri, MqBuilder mft)
			throws MessageException {
		brokers.put(brokerName, new MessageBroker(brokerName, uri, mft,
				hostType));
	}
	 ...
	 * @throws MessageException
	 */
	public Message createMessage(String brokerName, String hostType)
			throws MessageException {
		try {
			Message msg = this.getBroker(brokerName).getSession()
					.createMessage();
			setProperty(msg, Constants.MSG_HEAD_FROM,
					hostType + CommonUtils.getHostName());
			return msg;
	...
	public Message createMessage(String brokerName) throws MessageException {
		try {
			...
			if (hostType != null && !hostType.trim().isEmpty()) {
				setProperty(msg, Constants.MSG_HEAD_FROM, hostType
	...
	public static <V> V getProperty(Message message, String propertyName,
			V defaultValue) throws MessageException {
		...
		try {
			if (message.propertyExists(propertyName)) {
				String tName = defaultValue.getClass().getSimpleName();
	...
	@SuppressWarnings("unchecked")
	public static <V> V getProperty(Message message, String propertyName,
			Class<V> clazz) throws MessageException {
		if (propertyName == null)
			throw new MessageException("Given PropertyName is NULL.");
		if (clazz == null)
		...
		try {
			...
			throw new MessageException("Get Property[" + propertyName + "] as "
					+ clazz.getSimpleName() + " from message failed.", ex);
		}
		return (V) null;
	}
	 ...
	 *            Property Value
	 */
	public static <V> void setProperty(Message message, String key, V value)
			throws MessageException {
		if (key == null)
			throw new MessageException("Given PropertyName is NULL.");
		if (value == null)
			return;
		try {
			String tName = value.getClass().getSimpleName();
			if (tName.equals("String"))
			...
			throw new MessageException("Set Message property failed. Name:"
					+ key + ", value:" + value, ex);
		}
	}
	 ...
	 * @throws MessageException
	 */
	public Pipe bindHandlers(String brokerName, Type type, String name,
			String messageSelector, MessageListener listener,
			MessageFilter... filters) throws MessageException {
		return bindHandlers(brokerName, type, name, messageSelector, listener,
				null, filters);
	}
	...
	public Pipe bindHandlers(String brokerName, Type type, String name,
			String messageSelector, MessageListener listener,
			OutputStream tracker, MessageFilter... filters)
			throws MessageException {
		return getBroker(brokerName).bindHandlers(type, name, messageSelector,
				listener, tracker, filters);
	}
	 ...
	 * @param properties
	 */
	public void send(String brokerName, String pipeName, String content,
			HashMap<String, Object> properties) throws MessageException {
		try {
			...
			throw new MessageException("Send message fail,broker=" + brokerName
					+ ",pipe=" + pipeName + ",content=" + content
					+ ",properties=" + CommonUtils.toJson(properties), ex);
		}
	}
	 ...
	 * @throws MessageException
	 */
	public void send(String pipeName, String content,
			HashMap<String, Object> properties) throws MessageException {
		try {
			Pipe pipe = null;
			for (MessageBroker broker : brokers.values()) {
			...
			throw new MessageException("Send message fail,pipe=" + pipeName
	...
	public static String printMessage(Message message) {
		...
		try {
			...
			sb.append("  JMSMessageId:").append(message.getJMSMessageID())
					.append("\n");
			sb.append("  JMSCorrelationID:")
					.append(message.getJMSCorrelationID()).append("\n");
			sb.append("  JMSExpiration:").append(message.getJMSExpiration())
					.append("\n");
			sb.append("  JMSReplyTo:").append(message.getJMSReplyTo())
					.append("\n");
			...
			sb.append("  JMSDeliveryMode:")
					.append(message.getJMSDeliveryMode()).append("\n");
			sb.append("  JMSDestination:").append(message.getJMSDestination())
					.append("\n");
			sb.append("  JMSPriority:").append(message.getJMSPriority())
					.append("\n");
			...
			while (eu.hasMoreElements()) {
				...
				sb.append("  ").append(name).append(":")
						.append(getProperty(message, name, Object.class))
						.append("\n");
			}
			sb.append("} ");
		} catch (Exception ex) {
	...
	public static void printMessage(Message message, LogConnector log) {
		try {
			...
			while (eu.hasMoreElements()) {
				String name = eu.nextElement();
				log.info("  " + name + ":"
						+ getProperty(message, name, Object.class));
			}
			log.info("} ");
		} catch (Exception ex) {
			ex.printStackTrace();
