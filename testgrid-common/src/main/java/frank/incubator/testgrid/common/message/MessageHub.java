...
public class MessageHub {
	...
	/**
	 * Get Property from a message. The return value Type depends on the given default value Class type.
	 * 
	 * @param message message to be fetched.
	 * @param propertyName property name.
	 * @param defaultValue the default value, if left null, the fetch object class will be taken as Object.
	 * @return
	 * @throws MessageException
	 */
	...
	public static <V> V getProperty( Message message, String propertyName, V defaultValue ) throws MessageException {
		...
		try {
			...
			throw new MessageException( "Get Property[" + propertyName + "] as " + defaultValue.getClass().getSimpleName() + " from message failed.Default value:" + defaultValue, ex );
		}
	}
	
	/**
	 * Get Property from a message. The return value Type depends on the given Class type.
	 * @param message message message to be fetched.
	 * @param propertyName property name.
	 * @param clazz Give fetch property Class type.
	 * @return
	 * @throws MessageException
	 */
	@SuppressWarnings( "unchecked" )
	...
	/**
	 * Set property value to assigned message. Depends on what value given, invoke different api for message.
	 *  
	...
	/**
	 * Bind handler to special Queue or Topic on special Broker, with output supported.
	 * 
	 * @param brokerName
	 * @param type
	 * @param name
	 ...
	 * @throws MessageException
	 */
	public Pipe bindHandlers( String brokerName, Type type, String name, String messageSelector, MessageListener listener, OutputStream tracker, MessageFilter ... filters )
			throws MessageException {
		return getBroker( brokerName ).bindHandlers( type, name, messageSelector, listener, tracker, filters );
	}

	/**
	 * Dispose the whole MessageHub.
	 * Remove and clean all the brokers and related resources.
	 */
	public void dispose() {
		for( MessageBroker broker : brokers.values() ) {
			broker.dispose();
	...
	public void send( String brokerName, String pipeName, String content, HashMap<String, Object> properties ) throws MessageException {
		try {
			...
			throw new MessageException( "Send message fail,broker=" + brokerName + ",pipe=" + pipeName + ",content=" + content + ",properties=" + CommonUtils.toJson(properties), ex );
		}
	}
	
	/**
	 * Send message to assigned pipe, will automatically find all the broker which own the pipes with same pipeName.
	 * @param pipeName
	 * @param content
	 * @param properties
	 * @throws MessageException
