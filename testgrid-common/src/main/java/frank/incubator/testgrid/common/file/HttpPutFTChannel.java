...
public class HttpPutFTChannel extends FileTransferChannel {
	...
	/**
	 * Take it always true , cos only agent side support it. And this validation should be set explicitly depends on agent configuration.
	...
	public boolean receive( String token, Map<String, Long> fileList, File localDestDir, LogConnector log ) {
		...
		HttpFileTransferTarget target = new HttpFileTransferTarget( FileTransferMode.TARGET_HOST, getProperty( "postUrl", "" ), getProperty( "repoPath", File.class ), log.getOs() );
		try {
