...
/**
 * Very simple implementation of FileTransferTarget via directly socket transferring.
 * 
 * @author Wang Frank
 *
 */
			...
			if( response == Constants.VALIDATION_SUCC ) {
				for( String f : fileList.keySet() ) {
					...
					try {
						...
						fos.flush();
						files.add( file );
						log.info( "Target: finished receiving:"+f );	
					}finally {
						CommonUtils.closeQuietly( fos );
					}
				}
				...
				throw new Exception("Invalid token for fetching files from "+host+":"+port+", token:"+token);
			}
		}finally {
			CommonUtils.closeQuietly( os );
			CommonUtils.closeQuietly( in );
