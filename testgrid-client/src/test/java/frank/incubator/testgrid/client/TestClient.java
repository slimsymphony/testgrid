...
public class TestClient {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// TestStrategy.EXEC_ASAP, Paths.get( "client_workspace" ).toFile(),
			// System.out, descriptor, false , bds );
			TaskClient client = new TaskClient("tcp://localhost:61616", task, TestStrategy.EXEC_ASAP, Paths.get("client_workspace").toFile(), System.out,
					descriptor, true, false, bds);
			// client.setTaskTimeout(30000);
			client.setSerialized(false);
			client.setMaxConcurrentSize(2);
			client.begin();
	...
	private static DeviceRequirement getRequirement3() {
		DeviceRequirement dr = new DeviceRequirement();
		dr.setMain(Device.createRequirement(new HashMap<String, Object>(){{put("manufacturer","%NOT%LGE");}})); 
		return dr;
	}
	...
	private static Test createTest3() {
		...
		test.setExecutorParameters("-s ADB_HOME -d DEVICE_MAIN_SN -k APK_NAME -v");
		test.addResultFile("*.txt", true).addResultFile("*.mp4", false).addResultFile("*.xml", false);
		test.setArtifacts(createArtifacts());
	...
	private static Map<String, Long> createArtifacts() {
		...
		/*
		 * Path from = Paths.get( "client_workspace" ); try {
		 * Files.walkFileTree( from, new SimpleFileVisitor<Path>() {
		 * 
		 * @Override public FileVisitResult visitFile( Path file,
		 * BasicFileAttributes attrs ) throws IOException { ats.put(
		 * file.toFile().getName(), file.toFile().length() ); return
		 * FileVisitResult.CONTINUE; }
		 * 
		 * @Override public FileVisitResult preVisitDirectory( Path dir,
		 * BasicFileAttributes attrs ) throws IOException { return
		 * FileVisitResult.SKIP_SUBTREE; }
		 * 
		 * @Override public FileVisitResult postVisitDirectory( Path dir,
		 * IOException e ) throws IOException { if ( e == null ) { // return
		 * FileVisitResult.CONTINUE; return FileVisitResult.SKIP_SUBTREE; } else
		 * { // directory iteration failed throw e; } } } ); } catch (
