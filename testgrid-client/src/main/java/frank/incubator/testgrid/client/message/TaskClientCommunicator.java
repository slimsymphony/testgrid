			...
			if (client.getTask().getId().equals(taskID)) {
				...
				case Constants.MSG_TASK_ACCEPT:
					if (!client.isAllTestStart()) {
						...
						if (capacity.getAvailable() > 0 || capacity.getNeedWait() > 0) {
							...
							client.handleNewAccpetance(testId);
						} else {
							if(client.getAgentCandidates().containsKey(testId)) {
				...
				case Constants.MSG_TASK_CONFIRM:
					...
					if (reply == Constants.RESERVE_SUCC) {
						...
						if(test.getPhase().equals(Phase.PENDING)) {
							...
							for (String fn : test.getArtifacts().keySet()) {
								...
								if (af.exists() && af.isFile() && af.length() == test.getArtifacts().get(fn)) {
									...
									String reason = "File[" + af.getAbsolutePath()
											+ "] invalid, maybe not exists or not a file, or length[" + af.length()
							...
							FileTransferTask ftTask = new FileTransferTask(Constants.MSG_TARGET_CLIENT
									+ CommonUtils.getHostName(), from, taskID, testId, files);
							client.getFts().sendTo(ftTask);
							this.client.getTask().setPhase(Task.Phase.STARTED);
						}else {
							log.warn("Test["+testId+"] have been in phase:" + test.getPhase()+". No need to send reservation.");
						}
						...
						client.getTestSlots().put(client.getTestById(testId),
								new FakeTestListener(client.getTestById(testId)));
					} else {
				...
				case Constants.MSG_TEST_FINISHED:
					...
					if(itest != null) {
						...
						if(!missingFiles.isEmpty()) {
							log.warn("Test[" + testId + "] missing some result files. missing files:" + CommonUtils.toJson(missingFiles));
						}
						...
						if (result == Constants.MSG_TEST_SUCC && isSucc) {
							...
							if(result != Constants.MSG_TEST_SUCC) {
								log.error("Test["+testId+"] received unsuccess execution result:" + result);
							}else if(!isSucc) {
								log.error("Test["+testId+"] got result, but some of mandatory result files missing:" + CommonUtils.toJson(missingFiles));
							}
